...
import { MediaRemovalRequestStatus, MediaStatus, MediaType } from '@server/constants/media';
...
export class MediaRemovalRequest {
  ...
  @Column({ type: 'text', nullable: true, transformer: {
    ...
    from: (value?: string) => value ? JSON.parse(value) as number[] : undefined,
  }})
  ...
  public async executeRemoval(): Promise<void> {
    ...
    const specificServiceId = this.is4k
      ? media.serviceId4k
      : media.serviceId;
